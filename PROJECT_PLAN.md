...
| **R9** | Device removal mid-capture corrupts project | **L** (was M) | **H** | **Closed for the capture path 2026-09-25 in WP-04.** `source::Faults::unplug_after` injects it, and `capture_path.rs` plus `core/tests/capture_to_project.rs` assert the whole chain: the device goes quiet rather than ending the stream, one stream error is counted, the frames delivered before it went are byte-for-byte correct, the counters reach the file, and the project still validates. A cable cannot be pulled by CI, so it is pulled here instead. Impact stays H and the risk stays open until WP-05's writer is in the chain - today nothing is writing blocks while the device vanishes | Treat as stream error, finalise capture as `interrupted`, keep committed blocks |
...
all done, and so are S3, S4 and S5. WP-01 through WP-04 are built.*

1. **Next** - **WP-05, the persistence writer.** WP-04 is built, so the thing WP-05 was
   waiting for exists: a `Source` trait with a real CPAL implementation behind it *and*
   a simulated one, which means the writer can be developed and soaked in CI on a
   machine with no sound card and then run unchanged against a turntable. WP-02's schema
   has still never had a byte written into it by a capture - every block in every test
   today is synthetic - and WP-05 is what closes that gap. The 90-minute 24/192 soak is
   its exit criterion and the last unproven claim in the storage stack.
   **WP-03 and WP-04 both stay open on their own exit criteria** - the device matrix and
   ...
   the right failure, but it is not the cross-check the criterion asks for.
...
3. **Then** - WP-06, recovery. It is written against the same `Source` trait and the
   same `session` rows, so the kill-at-random-point suite can be driven by the simulated
   source too. WP-04 already left it the signal it needs: `finished_at IS NULL`, which
   is the absence of a write and therefore the one thing a crash cannot forge.
