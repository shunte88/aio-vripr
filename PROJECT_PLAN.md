...
- **New project should ask for artist, recording title and catalogue number** -
  ...
  new-project dialog is a metadata query that needs no further input: catalogue
...
| **M3** | *It finds tracks* | **Met 2026-09-26.** Parity measured at 97.6-99.7% of VRipr's boundaries on the labelled corpus, exact to the frame; every boundary carries provenance, confidence and its evidence | WP-11 | 10 | 89 |
...
*Updated 2026-09-26. The three actions that stood here - repo groundwork, S1, S2 - are
all done, and so are S3, S4 and S5. WP-01 through WP-13 are built; WP-06 meets M1,
WP-10 meets M2 and WP-11 meets M3.*

1. **Next** - **WP-14, export**, at weight 9 and unblocked by WP-13: a track now has
   an extent, a number, a title and a release behind it, which is exactly the input
   §33 asks a splitter for. It reads committed blocks and edit instructions and writes
   WAV and FLAC with tags, artwork and a naming template, all ported from VRipr. Its
   exit criterion is the same shape as WP-10's and can be met the same way - bit-exact
   extraction verified against the source blocks, with no device in the loop - plus one
   half that needs outside software: tags read back by a third-party reader. The pieces
   already on disk are `pcm::read` (WP-10's block reassembly, which is the only correct
   way to get samples out) and `track::positions`, which hands the naming template its
   `A1`/`1` strings without the template having to know about sides.

   **What WP-13 leaves for the transport and the UI.** `SKIP FORWARD` and `SKIP BACK`
   are still a fixed ten seconds. The boundaries to skip to now *exist as rows* -
   `track::boundaries` returns them in timeline order - so the remaining work is
   entirely in `vcw-core::playback`: ask the project for the next boundary past the
   cursor instead of adding 10 s. It was left undone on purpose rather than smuggled
   into WP-13, because §21's verb belongs to the transport and WP-16 is what decides
   whether a skip lands on a boundary or on the track start §33 pads out to.

   **The promotion floor is a policy, and policies get tuned.** `Policy::min_sources`
   is 2, which is WP-11's over-segmentation finding honoured, and on the real side it
   ...
   figure can be argued from a real record rather than from taste, and the UI (WP-16)
   is where a rejected-but-visible boundary should show up - the rows carry
   `confidence` and `sources` precisely so that it can.
   ...
   **WP-03 through WP-10 all stay open on portability.** The device
   ...
   Playback adds one of its own: the buffer size it asks a device for is honoured by
...
3. **Then** - **WP-15 and the Tauri shell**, which is the last thing between the
   engine and a user. Everything above it is reachable today only through `vcw`, and
   the CLI has been carrying that load deliberately: every verb WP-16's UI needs is
   already a function on a core crate with a test behind it, so the shell is a
   translation layer and not a second implementation. WP-15's exit criterion - core
   crates with zero Tauri dependency, enforced in CI - is the one that keeps it that
   way, and S3 already measured what the boundary costs (nothing; the main-thread
   waveform draw is the cost).
