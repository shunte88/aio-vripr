...
all done, and so are S3, S4 and S5. WP-01 through WP-08 are built, and with WP-06 the
M1 milestone is met.*

1. **Next** - **WP-09, the waveform pyramid.** Pure signal work over data the capture
   path already produces, and it needs no device: the simulated source and the 62-rip
   corpus at `/data2/source_rips` are enough to develop and verify against. It is also
   the one S3 identified as the real UI constraint, so the sooner it exists the sooner
   the rendering question can be measured rather than argued about. It has a head start
   it did not have last week: WP-05's writer already builds and persists summary
   pyramids for every block it commits, so WP-09 is a reader and a renderer over
   something the project already contains, not a builder from scratch.

   WP-08 left the fan-out ready for it. `vcw_audio::buffers::Tee` takes any number of
   taps and the meter uses one; the waveform worker is a second `tap()` call and a
   second thread, with no change to the capture path and no second reader of the device.
