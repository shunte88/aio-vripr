...
all done, and so are S3, S4 and S5. WP-01 through WP-11 are built; WP-06 meets M1,
WP-10 meets M2 and WP-11 meets M3.*

1. **Next** - **WP-12, metadata**, at weight 9 and needing no device at all: a provider
   trait, Discogs, MusicBrainz, genre normalisation, artwork, caching, rate limits,
   timeouts and cancellation. Its exit criterion is that the application stays fully
   usable with networking disabled, which makes the tests fixture-backed and offline by
   construction, and §39's rule that no credential is ever written into a project file
   is a constraint on the design rather than a check at the end. **WP-13, the vinyl data
   model and editing**, is the alternative, and now unblocked: WP-11 publishes boundaries
   and deliberately writes no tracks, so WP-13 is where a boundary becomes one - and
   where §24's locking, which `resolve` already honours, gets something to lock.

   **What WP-11 leaves for WP-13 and the UI.** `SKIP FORWARD` and `SKIP BACK` are still
   a fixed ten seconds; the boundaries to skip to now exist but nothing stores them, so
   the transport has nothing to ask. And the HMM over-segments a real side badly - 133
   candidate tracks on a 26-minute rip where the level and flatness detectors agree on
   three - which is VRipr's behaviour reproduced faithfully, not a defect in the port.
   It is the reason `resolve` reports agreement and the reason a one-detector boundary
   comes back at confidence 0.50: the UI has what it needs to show the difference, and
   WP-13 should not promote a boundary no second detector saw.
