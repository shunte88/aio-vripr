...
all done, and so are S3, S4 and S5. WP-01 through WP-10 are built; WP-06 meets the M1
milestone and WP-10 meets M2.*
...
1. **Next** - **WP-12, metadata**, at weight 9 and needing no device at all: a provider
   ...
   is a constraint on the design rather than a check at the end. **WP-11, editing, and
   WP-13, boundaries** are the alternative, and both build directly on what WP-10 just
   landed; WP-13 is also what turns `SKIP FORWARD` and `SKIP BACK` from a fixed ten
   seconds into the next boundary either way, which is what §21 is asking for.
